...
-- >>> 20260608000010_push.sql >>>
-- ============================================================================
-- NutriAI Â· 0010 Â· Suscripciones de notificaciones push (Web Push)
-- Ejecuta este archivo en el SQL Editor de Supabase (una vez).
-- ============================================================================
...
-- >>> 20260608000011_gym_reminder.sql >>>
-- ============================================================================
-- NutriAI Â· 0011 Â· Recordatorio de pago de la mensualidad del gym
-- Ejecuta este archivo en el SQL Editor de Supabase (una vez).
-- ============================================================================
...
-- >>> 20260608000012_set_logs.sql >>>
-- ============================================================================
-- NutriAI Â· 0012 Â· Registro de series (peso/reps) y rÃ©cords personales
-- Ejecuta este archivo en el SQL Editor de Supabase (una vez).
-- ============================================================================
...
-- RetenciÃ³n: incluir los registros de series en la limpieza de 90 dÃ­as
create or replace function public.delete_old_data()
returns void
language plpgsql
security definer set search_path = public
...
-- >>> 20260608000013_add_water_fn.sql >>>
-- ============================================================================
-- NutriAI Â· 0013 Â· Suma atÃ³mica de agua (evita perder toques rÃ¡pidos)
-- Ejecuta este archivo en el SQL Editor de Supabase (una vez).
-- ============================================================================
...
-- >>> 20260608000014_payments.sql >>>
-- ============================================================================
-- NutriAI Â· 0014 Â· Pagos / ingresos (base para SaaS)
-- Registra cada pago de mensualidad (manual por Bre-B) para llevar ingresos,
-- historial y mÃ©tricas. Ejecuta este archivo en el SQL Editor de Supabase.
-- ============================================================================

create table if not exists public.payments (
  id            uuid primary key default gen_random_uuid(),
  -- Si se elimina el usuario, conservamos el pago para el histÃ³rico de ingresos.
  user_id       uuid references auth.users(id) on delete set null,
  user_email    text,                       -- copia para el histÃ³rico
  amount        numeric(12,2) not null default 0 check (amount >= 0),
  currency      text not null default 'COP',
  plan          text not null default 'ai', -- 'general' | 'ai'
  method        text not null default 'bre-b',
  reference     text,                        -- nÂº de comprobante / nota
  period_start  date,
  period_end    date,
...
-- Cada usuario puede ver SUS pagos (para un futuro historial de facturaciÃ³n).
-- El panel admin usa la service_role (salta RLS), asÃ­ que no necesita polÃ­tica.
drop policy if exists "payments_select_own" on public.payments;
create policy "payments_select_own" on public.payments
  for select using (auth.uid() = user_id);


-- >>> 20260608000015_payment_proofs.sql >>>
-- ============================================================================
-- NutriAI Â· 0015 Â· ConfirmaciÃ³n de pagos (comprobantes)
-- El usuario envÃ­a su comprobante â†’ pago 'pending' â†’ el admin lo confirma.
-- Ejecuta este archivo en el SQL Editor de Supabase (despuÃ©s de 0014).
-- ============================================================================
...
-- Bucket privado para los comprobantes (sÃ³lo el dueÃ±o escribe; el admin lee
-- con la service_role). ConvenciÃ³n de ruta: "<user_id>/<archivo>".
insert into storage.buckets (id, name, public)
